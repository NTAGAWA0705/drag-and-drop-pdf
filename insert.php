...
if (isset($_POST['submit_btn'])) { // If isset upload button or not
    ...
    for ($i = 0; $i < (count($files) - 1); $i++) {
        ...
        $file_name = $_FILES["file"]["name"][$i]; // Get uploaded file name
        $file_temp = $_FILES["file"]["tmp_name"][$i]; // Get uploaded file temp
        $file_size = $_FILES["file"]["size"][$i]; // Get uploaded file size

        $file_title = $conn->real_escape_string($_POST["file_title"]);
        $file_ext = pathinfo($file_name, PATHINFO_EXTENSION);
        $file_new_name = date("dMYHis") . strtolower(str_replace(' ', ' ', $file_name)) . "_$i" . $file_ext;
        ...
        $sql = "INSERT INTO file (file_name, user_id, file_title)
                    VALUES ('$file_new_name', 1, '$file_title')";
        // saving query in the databases
        $result = mysqli_query($conn, $sql);
        if ($result) {
            move_uploaded_file($file_temp, $location . $file_new_name);
            echo show_alert("The file was successfully uploaded", "success");
        } else {
            echo show_alert("There was an error while saving database data<br>
                " . $conn->error . "
            ", "error");
        }
    }
